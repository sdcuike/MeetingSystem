...
BEGIN_MESSAGE_MAP(CChatClientView, CRichEditView)
	...
	ON_COMMAND(ID_WB_POPUP, OnWbPopup)
	//}}AFX_MSG_MAP
...
BOOL CChatClientView::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	//  the CREATESTRUCT cs
	whiteboard=new CWhiteBoard();
	if (whiteboard!=NULL)
	{
		BOOL ret=whiteboard->Create(IDD_DIgWhiteborad);
		if (!ret)
		{
			AfxMessageBox("Error creating Dialog");
		}
		whiteboard->ShowWindow(SW_HIDE);
	} 
	else
	{
...
void CChatClientView::OnWbPopup() 
{
	// TODO: Add your command handler code here
	whiteboard->ShowWindow(SW_SHOW);
