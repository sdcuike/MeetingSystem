	...
	TRY
	{
		...
		if(msg.type == -1)
		{
			...
			if(found==false)
			{
				...
				Message1(msg.from);
				Message1("进入多人会有系统\r\n");
				CMainFrame *temp=(CMainFrame*)AfxGetApp()->m_pMainWnd;
				CString strIp;
				UINT ipLeng;
				pSocket->GetSockName(strIp,ipLeng);
		        temp->m_wndOnline.m_ListCtrl->AddItem(0,pSocket->name.GetBuffer(pSocket->name.GetLength()),
					strIp.GetBuffer(ipLeng));
			}
		}
		if(msg.type == -2)
		{
			...
			Message1("离开会议系统\r\n");
			CMainFrame *temp=(CMainFrame*)AfxGetApp()->m_pMainWnd;
			temp->m_wndOnline.m_ListCtrl->Remove(msg.from.GetBuffer(msg.from.GetLength()));
		}
		int i = msg.type;
		if(i >= 0 && i <= 32&& i!=30){
